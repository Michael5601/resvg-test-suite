...
svg2_files = []
if is_svg2_only:
    files = list(Path('eclipse-tests-no-duplicates').rglob('*.svg'))
...
rows = []
with open('custom - no-duplicate.csv', 'r') as f:
...
with open('chart_custom-no-duplicate.json', 'w') as f:
...
with open('group_results_custom-no-duplicate.json', 'w') as f:
    f.write(json.dumps(group_data, indent=4))

if is_svg2_only:
    out_path = 'site/images/chart-svg2_custom-no-duplicate.svg'
else:
    out_path = 'site/images/chart_custom-no-duplicate.svg'

try:
    subprocess.check_call(['./barh', 'chart_custom-no-duplicate.json', out_path])
